...
- **Widget**: separate ≤60KB iframe bundle + <3KB `embed.js` loader + `@chatform/react` npm package for self-hosting; postMessage bridge; SSE reconnect with zero answer loss.
...
| M8 | API keys UX + `/v1` headless API (sync + SSE chat, idempotency) + webhook dispatch/retries/DLQ UI + embed.js/widget + `@chatform/react` alpha | curl headless session completes form; HMAC verified; revoked key rejected ≤60s |
...
## 10. Status log (build session 1)
- DONE: monorepo scaffold; form-schema pkg (22 tests green: zod FormDoc, deterministic logic engine w/ scoped `from` goto rules, per-type validators, lint); full D1 schema + migration; Hono API with health + public chat routes; **SessionDO FSM (template mode)** — SSE durable replay, greeting/question/validation/branching/endings, partial-save projection to D1, idle-alarm abandonment, Turnstile hook, R2/queues/AE bindings; **browser-verified E2E** (seeded form → chat → completed submission w/ normalized answers); Next.js 16 + Tailwind v4 + shadcn (16 components) + warm/orange token system; functional chat client (chips, stars, NPS scales, text/email inputs); **OpenAPI spec via hono-openapi at /openapi.json + Scalar at /docs**; **orval → TanStack Query hooks codegen** (`pnpm gen:api`, mutator pattern, ApiProvider wired).
- NEXT: M1 Better Auth (+org plugin owns members/invites/roles; admin plugin) → M3 forms CRUD (described w/ hono-openapi, regen orval) → builder UI → M5 AI agent layer → M6 results/analytics.
