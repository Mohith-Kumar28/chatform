...
export function PeopleSection() {
  ...
  if (isPending) {
    return (
      <>
        ...
        <div className="grid gap-6 @3xl/settings:grid-cols-[1.6fr_1fr]">
  ...
  return (
    <>
      {/* The workspace's name is no longer repeated here — the rail beside this
          pane is headed with it, so saying it twice on one screen was the kind of
          duplication this consolidation exists to remove. The counts stay. */}
      ...
      {/*
        Split on the *pane's* width, not the viewport's.

        With the rail present this pane is ~830px inside a 1152px page; on a phone
        it is the whole page. A viewport breakpoint gets both wrong — it would
        stack when there is room and, worse, keep two columns when the invite card
        has only ~280px, which is not enough for the role control, the seat meter
        and the plan padlock that all live in it.
      */}
      <div className="grid items-start gap-6 @3xl/settings:grid-cols-[1.6fr_1fr]">
        <div className="space-y-6">
          <Card>
            {/* No title: the section header two lines up already says "People",
                and a card repeating its own heading is the kind of duplication
                this consolidation exists to remove. The sentence stays, because
                it is the only thing that says pending invitations are in the
                list rather than in a second one. */}
            <CardHeader>
              <CardDescription>
                Everyone with access to this workspace, and anyone on the way in.
              </CardDescription>
            </CardHeader>
