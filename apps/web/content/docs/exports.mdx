...
Finished and unfinished responses are not the same kind of row, so the
dashboard's Download keeps them apart: it hands you an `.xlsx` with a
**Completed** tab and an **Unfinished** tab over the same columns. Its `.csv`
route — which has no tabs, and which the spreadsheet feed still uses — puts
every completed response first and the unfinished ones after instead. The API's
`csv` is one flat table with a `status` column, unchanged.
