...
dashboard's download keeps them apart: the `.xlsx` gets a **Completed** tab and
an **Unfinished** tab over the same columns, and the `.csv` — which has no tabs
— puts every completed response first and the unfinished ones after. The API's
