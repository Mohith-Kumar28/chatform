...
| Scope | Why |
| --- | --- |
| `form:read`, `form:write`, `form:publish` | Read, edit and publish forms, and use templates |
...
| `file:read` | Resolve file-upload answers |

`ai:generate` is deliberately **not** in that list. It is the one scope that spends
money — a generation bills your plan's AI allowance — and an assistant can author a
form for free by reading `list_blocks` and calling `create_form`. Add it only if you
specifically want Chatform's own designer model available to the agent.
...
Thirty, which is a budget rather than an accident: clients cap how many tools
they will load — Cursor at forty — and that ceiling is shared with every other
...
| `list_templates` | `form:read` | The official template catalogue, or one template's document |
| `use_template` | `form:write` | Start a draft from a template |
| `list_form_versions` | `form:read` | Published versions, with the responses behind each, and diffs |
| `restore_form_version` | `form:write` | Roll the draft back to a published version |
| `generate_form_with_ai` | `ai:generate` | Draft a document with Chatform's own model — **costs money** |
...
| `create_spreadsheet_feed` | `response:export` | A live CSV URL Sheets or Excel can re-read on a schedule |
...
- **Driving a live conversation** — opening a chat session and exchanging messages
  ...
  assistant: `submit_response` records answers directly without pretending to be a
  respondent. If you want named tools for testing a form conversationally, that is a
  reasonable ask.
- **Incremental response building** — recording answers one at a time, then
  completing or abandoning `GET /v1/responses/{id}/next` and its siblings.
  `submit_response` does the one-shot version, which is what an import or an agent
...
- **Deleting responses in bulk**, sending a synthetic webhook test, and moving a form
  between workspaces. Each has a dashboard route and no API one yet — tracked in
  `tooling/check-spec-coverage.ts`, which now fails the build if a new dashboard
  capability appears without either a `/v1` equivalent or a written reason.
