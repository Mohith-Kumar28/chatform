...
That shape is universal, including when the request body did not match the
schema. A malformed body is `400 invalid_request`, and `issues[]` carries one
entry per field with the `path` that failed:

```json
{
  "error": {
    "code": "invalid_request",
    "message": "Ref: This is missing (and 1 other field)",
    "issues": [
      { "path": "ref", "code": "invalid_type", "message": "Ref: This is missing" },
      { "path": "value", "code": "invalid_type", "message": "Value: This is missing" }
    ],
    "request_id": "req_9f2c…"
  }
}
...
`file_too_large` · `name_required` · `payment_pending` · `payment_unverified` ·
`incomplete` · `consent_required` · `unsupported`
...
| Code | Meaning |
| --- | --- |
| `invalid_request` | The body or query did not match the schema; `issues` names each field |
| `malformed_json` | A JSON content type with a body that is not JSON, including an empty one |
| `not_published` | The form exists but has no published version; read the draft with `?view=document` |
