...
- **Only gateway payments are verified.** A payment block on a link or UPI QR
  records that the respondent said they paid, so `verified` is `false` and you
  reconcile against your processor. [Verified checkout](/docs/payments) on your
  own Razorpay, Cashfree or Stripe account is the one kind a gateway confirms.
