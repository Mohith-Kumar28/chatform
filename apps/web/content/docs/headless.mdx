...
it is given. A six-digit code goes to the address or number, and the session
emits `verify_required` — `{ ref, channel, sentTo, sentAt }` — with the same
thing on `status.pendingVerification` for callers that poll rather than stream.

Send the code back as an ordinary message. While a challenge is outstanding the
session reads what arrives as the code and nothing else, so there is no separate
route and no token to carry:
...
`invalid_code`, and the challenge stays open: try again, `resend_code`, or
`change_answer` to drop it and ask the question afresh. On success the answer is
recorded exactly as an unverified one would have been, `verify_settled` fires,
...
is a different claim, and gets its own code.
</Callout>
