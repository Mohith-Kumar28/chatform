...
New env: `GOOGLE_RESPONDENT_CLIENT_ID` (API, for audience validation), `NEXT_PUBLIC_GOOGLE_RESPONDENT_CLIENT_ID` (web, for the button), `FIREBASE_PROJECT_ID` (API) with the matching `NEXT_PUBLIC_FIREBASE_*` (web). With no Google client id the button degrades to a plain "not available" line rather than breaking the card.
